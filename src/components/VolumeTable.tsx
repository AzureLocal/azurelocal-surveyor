...
export default function VolumeTable() {
  ...
  function handleAdd() {
    ...
    addVolume({ id: nextId(), name: newName.trim(), plannedSizeTB: newSizeTiB, resiliency: newResiliency })
    setNewName('')
    setNewSizeTiB(1)
  }
  ...
  function startEdit(v: typeof summary.volumes[0]) {
    setEditId(v.id)
    setEditName(v.name)
    setEditSizeTiB(v.calculatorSizeTB)   // stored value is already the TiB the user entered
    setEditResiliency(v.resiliency)
  }

  function commitEdit() {
    if (!editId) return
    updateVolume(editId, { name: editName.trim() || editName, plannedSizeTB: editSizeTiB, resiliency: editResiliency })
    setEditId(null)
  }
    ...
    <div className="space-y-4">
      ...
      <div>
        <div className="flex justify-between text-xs text-gray-500 mb-1">
          ...
          <span>
            {summary.utilizationPct}% of {capacity.effectiveUsableTB} TB effective usable
            <span className="ml-1 text-gray-400">
              (≈ {(capacity.effectiveUsableTB * 0.909495).toFixed(1)} TiB in WAC)
            </span>
          </span>
        </div>
          ...
          {summary.volumes.length > 0 && (
            <tfoot className="border-t-2 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
              ...
              <tr className="border-t border-gray-100 dark:border-gray-800">
                <td colSpan={2} className="px-4 py-1.5 text-xs text-gray-400">Remaining effective usable</td>
                <td colSpan={2} className="px-4 py-1.5 text-right text-xs font-mono text-gray-500">
                  {summary.remainingUsableTB} TB ({(summary.remainingUsableTB * 0.909495).toFixed(2)} TiB)
                </td>
                <td />
              </tr>
            </tfoot>
